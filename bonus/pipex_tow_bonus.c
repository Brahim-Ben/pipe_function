...
/*   Updated: 2025/03/06 21:22:52 by bbenaali         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child2_process(int *fd, char **av, char **env, int num)
{
	...
	f = open_files2_bonus(av[4], fd);
	splited_path = ft_split(get_path(env), ':');
	arg = ft_split(av[3], ' ');
	if (!arg || !splited_path)
		(for_close_files_bonus(f, fd, arg, splited_path), error_malloc(), exit(1));
	if (num == 2)
	...
	if (dup2(fd[0], 0) == -1 || dup2(f, 1) == -1)
		(for_close_files_bonus(f, fd, arg, splited_path), error_dup(), exit(1));
...
void	child1_process(int *fd, char **av, char **env, int num)
{
	...
	f = open_files1_bonus(av[1], fd);
	splited_path = ft_split(get_path(env), ':');
	argg = ft_split(av[2], ' ');
	if (!argg || !splited_path)
		(for_close_files_bonus(f, fd, argg, splited_path), error_malloc(), exit(1));
	if (num == 2)
	...
	if (dup2(f, 0) == -1 || dup2(fd[1], 1) == -1)
		(for_close_files_bonus(f, fd, argg, splited_path), error_dup(), exit(1));
...
void pip_bonus(int ac, char *av[], char **env)
{
	...
	if (ac != 5)
		write(1, "Error: You should enter 4 parameters!\n", 38), exit(1);
	num[0] = check_the_command_one(env, av[2]);
	num[1] = check_the_command_one(env, av[3]);
	if (pipe(fd) == -1)
		write(1, "Error: Pipe function failed!\n", 29), exit(1);
	id = fork();
	if (id == -1)
		write(1, "Error: Fork function failed!\n", 29), exit(1);
	if (id == 0)
	...
	if (waitpid(id, NULL, 0) == -1 || waitpid(id2, NULL, 0) == -1)
		write(1, "Error: waitpid function failed!\n", 29), exit(1);
}
